...
ReactDOM.render( // метод получает на вход ДВА параметра:
  <TodoApp store={store} />, // то, что нужно зарендерить (Так как приложение не имеет
  // своего состояния, в качестве пропса передаём ему store)
  document.querySelector('#app') // и куда на странице это поместить (в тег с id "app")
)
